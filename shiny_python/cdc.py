...
counties = df_kidney['LocationName'].unique()
...
def server(input, output, session):
    ...
    def avg_data_value():
        ...
        avg_value = df_kidney[df_kidney['LocationName'] == selected_county]['Data_Value'].mean()
        return f"Average kidney Disease Age-adjusted Prevalence for {selected_county}: {avg_value:.2f}%"

    @output
    @render.plot(alt="kidney Disease Age-adjusted Prevalence Bar Chart")
    def bar_chart():
        overall_avg = df_kidney['Data_Value'].mean()
        selected_county_avg = df_kidney[df_kidney['LocationName'] == input.county()]['Data_Value'].mean()
